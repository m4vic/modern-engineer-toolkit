# 🤖 Robotics Roadmap  
*Modern Engineer Toolkit — Robotics Track*

A modern, simulation-first roadmap for mastering robotics: ROS2, manipulation, motion planning, SLAM, computer vision, autonomous systems, and robot learning.

---

## 🧭 Overview  
Robotics integrates **mechanical systems**, **electronics**, **real-time software**, **control theory**, **simulation**, and **AI-driven perception & planning**.  
This roadmap progresses from prerequisites → beginner → intermediate → advanced → specialization.

Cross-linked with:

- [Fundamentals](../Fundamentals/README.md)
- [Machine Learning](../MachineLearning/README.md)
- [Deep Learning](../DeepLearning/README.md)
- [GenAI / LLMs](../GenerativeAI/README.md)
- [Quant](../QuantFinance/README.md)
- [AGI](../AGI/README.md)

---

## 📘 Prerequisites  
(Complete these in the **Fundamentals** section of the repo)

- Math: linear algebra, calculus, probability  
- Physics: classical mechanics, forces, motion  
- Programming: Python + basic C++  
- Electronics: sensors, microcontrollers  

→ Covered in: **[Fundamentals](../Fundamentals/README.md)**

---

# 🛠️ Phase 0 — Foundation (1–2 Months)

<details>
<summary><strong>Mathematics & Physics Resources</strong></summary>

| Resource | Type | Cost | Link |
|---------|------|------|------|
| Khan Academy Physics | Course | Free | https://www.khanacademy.org/science/physics |
| Khan Academy Linear Algebra | Course | Free | https://www.khanacademy.org/math/linear-algebra |
| 3Blue1Brown Linear Algebra | Videos | Free | https://www.3blue1brown.com/topics/linear-algebra |
| MIT OCW: Classical Mechanics | Course | Free | https://ocw.mit.edu |

</details>

<details>
<summary><strong>Programming & Electronics Resources</strong></summary>

| Resource | Type | Cost | Link |
|---------|------|------|------|
| Arduino Official Channel | YouTube | Free | https://www.youtube.com/c/Arduino |
| SparkFun Electronics | Tutorials | Free | https://www.youtube.com/c/SparkFun |
| Adafruit Industries | Tutorials | Free | https://www.youtube.com/c/AdafruitIndustries |
| Python (W3Schools) | Web Tutorial | Free | https://www.w3schools.com/python |
| C++ Basics | Web | Free | Multiple free sources |

</details>

<details>
<summary><strong>Robotics Hardware Introduction</strong></summary>

| Resource | Type | Focus | Link |
|---------|------|--------|------|
| James Bruton | YouTube | DIY robots, mechanisms | https://www.youtube.com/c/JamesBruton |
| DroneBot Workshop | YouTube | Sensors, Arduino, Pi | https://www.youtube.com/c/Dronebotworkshop |
| How to Mechatronics | YouTube | Arduino, mechanics | https://www.youtube.com/c/HowToMechatronics |
| Jeremy Fielding | YouTube | Motors, actuators | https://www.youtube.com/c/JeremyFielding |

</details>

---

# 🚦 Phase 1 — Beginner Robotics (2–3 Months)

### Core Skills  
- Kinematics basics  
- Mobile robots  
- Sensors & microcontrollers  
- Simple simulation in Webots/Gazebo  

<details>
<summary><strong>Free University Courses</strong></summary>

| Course | Provider | Level | Link |
|--------|----------|--------|------|
| Introduction to Robotics (2.12) | MIT OCW | Beginner | https://ocw.mit.edu/courses/2-12-introduction-to-robotics-fall-2005/ |
| Introduction to Robotics (CS223A) | Stanford SEE | Beginner | https://see.stanford.edu/course/cs223a |
| Hello Real World with ROS | edX (Delft) | Beginner | https://www.edx.org/learn/robotics/delft-university-of-technology-hello-real-world-with-ros-robot-operating-system |

</details>

<details>
<summary><strong>YouTube Channels</strong></summary>

| Channel | Focus | Subscribers |
|--------|--------|-------------|
| James Bruton | DIY robots | 1.38M |
| Jeremy Fielding | Motors, actuators | 1.14M |
| SparkFun | Sensors | 242K |
| Adafruit | Electronics | 460K |

</details>

<details>
<summary><strong>Simulation Platforms</strong></summary>

| Platform | Features | Link |
|----------|----------|------|
| Webots | Open-source, URDF support | https://cyberbotics.com |
| Gazebo | Physics + ROS integration | https://gazebosim.org |
| CoppeliaSim | Multi-robot physics | https://www.coppeliarobotics.com |

</details>

---

# 🧩 Phase 2 — Intermediate Robotics (3–6 Months)

### Core Skills  
- ROS / ROS2  
- Manipulators & kinematics  
- Motion planning  
- Perception fundamentals  
- Dynamics & control  

<details>
<summary><strong>Free Advanced Courses</strong></summary>

| Course | Provider | Link |
|--------|----------|------|
| Programming for Robotics (ROS) | ETH Zürich | https://rsl.ethz.ch/education-students/lectures/ros.html |
| Robotic Manipulation (6.4210) | MIT | https://ocw.mit.edu/courses/6-4210-robotic-manipulation-fall-2022/ |
| Autonomous Mobile Robots | ETH/edX | edX platform |

</details>

<details>
<summary><strong>ROS & Dev Resources</strong></summary>

| Resource | Type | Link |
|----------|------|------|
| ROS Docs | Tutorials | https://www.ros.org |
| ROS2 Docs | Official | https://docs.ros.org |
| The Construct | Courses | https://www.theconstruct.ai |
| Mastering Gazebo | Course | The Construct |
| ROS2 Tutorials Repo | GitHub | https://github.com/sagar16812/Getting-Started-with-ROS2-A-Tutorial-Series |

</details>

<details>
<summary><strong>Key GitHub Repositories</strong></summary>

| Repo | Purpose | Link |
|------|----------|------|
| awesome-robotics | Curated list | https://github.com/search?q=awesome-robotics |
| awesome-robotics-projects | Open-source robots | https://github.com/mjyc/awesome-robotics-projects |
| navigation2 | ROS2 navigation | https://github.com/ros-planning/navigation2 |
| moveit | Motion planning | https://github.com/moveit/moveit |
| librealsense | Depth cameras | https://github.com/IntelRealSense/librealsense |
| matlab-robotics | MATLAB robotics | https://github.com/mathworks-robotics/awesome-matlab-robotics |

</details>

---

# 🧠 Phase 3 — Advanced Robotics (6–12 Months)

### Core Skills  
- Full autonomy stack  
- SLAM & mapping  
- Vision-based control  
- Reinforcement learning for robotics  
- High-fidelity simulation  

<details>
<summary><strong>Advanced Courses</strong></summary>

| Course | Provider | Link |
|--------|----------|------|
| MIT MicroMasters in Robotics | MIT/edX | edX platform |
| SLAM & Navigation (ROS2) | ROS2 Docs | https://docs.ros.org/humble/p/nav2/ |
| RL for Robotics | Various | Multiple |
| OpenCV University Bootcamps | OpenCV | https://opencv.org/university/free-courses/ |

</details>

<details>
<summary><strong>Specialized Topics</strong></summary>

### Computer Vision  
- OpenCV University (free bootcamps)  
- PyImageSearch CV crash course  
- YOLOv8 (Ultralytics)

### Deep Learning & Perception  
- PyTorch, TensorFlow  
- OpenCV DNN module  

### SLAM  
- SLAM Toolbox  
- ORB-SLAM3  
- Hector SLAM  

</details>

---

# 🦾 Phase 4 — Specialization (Year 2+)

<details>
<summary><strong>Paid Professional Certifications</strong></summary>

| Certification | Provider | Focus |
|---------------|----------|--------|
| Udacity Robotics Nanodegree | Udacity | ROS, Gazebo, navigation |
| FANUC Operator | FANUC | Industrial robotics |
| CIRP | ATMAE | Industrial robot programming |
| KUKA College | KUKA | Robot programming |

</details>

<details>
<summary><strong>Specialization Areas</strong></summary>

### Manipulation  
- MIT 6.4210  
- MoveIt tutorials  
- Dexterous hand projects  

### Humanoids & Legged Robots  
- Berkeley Humanoid Lite  
- Boston Dynamics papers  

### Autonomous Vehicles  
- ETH Autonomous Mobile Robots  
- TurtleBot3 + ROS2  
- NASA Open Source Rover  

</details>

---

# 🧪 Projects (Beginner → Advanced)

### Beginner  
- Line-following robot  
- Ultrasonic obstacle avoider  
- Two-wheel differential drive simulation  
- Basic robotic arm with IK demo  

### Intermediate  
- ROS2 mobile robot  
- LIDAR-based mapping  
- MoveIt pick-and-place  
- Vision-based object tracking  

### Advanced  
- Full SLAM + Navigation2 pipeline  
- Quadruped gait controller  
- Vision-language robot with YOLOv8 + LLMs  
- Drone autonomy with PX4 + Gazebo  

---

# 📅 Suggested Timeline

### Months 1–2: Foundations  
- Python, C++ basics  
- Arduino basics  
- Start Webots/Gazebo simulation  

### Months 3–4: Beginner Robotics  
- Stanford CS223A or MIT 2.12  
- Build first simulated robot  
- Learn fundamental kinematics  

### Months 5–6: ROS2 + Intermediate Concepts  
- Delft “Hello Real World with ROS”  
- Navigation basics  
- Build a simulated robot with sensors  

### Months 7–12: Specialization Path  
Choose one: Manipulation, Mobile Autonomy, Vision, UAVs  

### Year 2+  
- Industry certifications  
- Research projects  
- Open-source contributions  

---

# 🧩 Cross-links  
- Math/Physics → [Fundamentals](../Fundamentals/README.md)  
- ML for perception → [Machine Learning](../Machine-Learning/README.md)  
- DL for vision → [Deep Learning](../Deep-Learning/README.md)  
- LLM robotics → [GenAI](../GenAI/README.md)

---

# ✔️ End of Robotics Roadmap  
*Part of the Modern Engineer Toolkit*
